...
Two independent checks (Nivel 4 of the evidencing design):
  ...
  1. Pinned-SHA consistency — the BCQuality pin lives in THREE places that must
     agree: `pin` in tools/bcquality/bcquality.pin (canonical) and the
...
# The pin file that ships beside this script is the single source of truth for the
# BCQuality source and version — the same file both precondition hooks read.
PIN_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "bcquality.pin")

SHA_RE = re.compile(r"^\s*pin\s*=\s*([0-9a-f]{40})\s*$", re.MULTILINE)
# Legacy: a project's aldc.yaml may still override the shipped pin.
ALDC_SHA_RE = re.compile(r'pinnedCommit:\s*"?([0-9a-f]{40})"?')
ALDC_HOME_RE = re.compile(r'^\s*home:\s*"?([^"\n#]+?)"?\s*(?:#.*)?$', re.MULTILINE)
...
def pinned_sha(root: str) -> str | None:
    """The pinned commit: a project's aldc.yaml wins, otherwise the shipped pin file."""
    text = _read(os.path.join(root, "aldc.yaml"))
    if text:
        m = ALDC_SHA_RE.search(text)
        if m:
            return m.group(1)
    text = _read(PIN_FILE)
    if text:
...
def bcquality_home(root: str) -> str | None:
    """Clone location: $BCQUALITY_HOME, a project aldc.yaml override, else the hook default."""
    env = os.environ.get("BCQUALITY_HOME")
    if env:
        return env
    text = _read(os.path.join(root, "aldc.yaml"))
    if text:
        m = ALDC_HOME_RE.search(text)
        if m:
            return m.group(1).strip()
    return os.path.join(os.path.expanduser("~"), ".claude", "bcquality")
...
def main() -> int:
    ...
    # The hooks read url/ref/pin from bcquality.pin (single source of
    ...
    pinned = pinned_sha(root)
    if pinned:
        notes.append(f"BCQuality pinned to {pinned} (bcquality.pin, or an aldc.yaml override).")
    else:
        notes.append("BCQuality not pinned (tracking a branch/tag) - pin coherence skipped.")

    # --- Resolve the external BCQuality clone (multi-root; no in-repo submodule) ---
    bcq_root = args.bcquality_root or bcquality_home(root)
    bcq_abs = None
    if bcq_root:

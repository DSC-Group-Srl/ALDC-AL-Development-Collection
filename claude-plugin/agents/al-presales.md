...
- **Architecture design** → Use the Task tool to delegate to agent `al-architect` with context: "Design solution architecture based on the estimation and requirements"
- **TDD implementation** → Use the Task tool to delegate to agent `al-conductor` with context: "Implement with TDD orchestration based on the approved specification"

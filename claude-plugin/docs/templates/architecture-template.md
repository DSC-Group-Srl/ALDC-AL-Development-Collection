<!-- ALDC Core Template — IMMUTABLE. Do NOT edit this file directly.
     Copy to requirements/{req_name}/{req_name}.architecture.md and fill in. -->
...
{Explain the chosen pattern (event-driven extension, stored fields vs FlowFields, etc.) and *why*. Include Mermaid diagrams showing data flow, process flow and/or object relationships — **1 for MEDIUM, up to 3 for HIGH** (only as many as genuinely clarify something; a diagram that just restates the prose is not worth its tokens). Diagrams must add information that prose cannot — do not duplicate the diagram in a table.}
...
{**Minimum 1 for MEDIUM, minimum 3 for HIGH** — only decisions with a genuine alternative that was considered and rejected; don't pad to hit a count. For each, document the problem, the chosen option, alternatives rejected and the rationale. This section is what makes the document auditable months later.}
...
{**Minimum 1 for MEDIUM, minimum 3 for HIGH** — only real risks specific to this feature, not generic boilerplate ("tests might fail"). Include likelihood, impact and explicit mitigation.}
...
- **Decision/risk/diagram counts scale by tier, not a flat minimum**: MEDIUM needs at least 1 Technical Decision, 1 Risk, and 1 diagram in §3 (if any diagram is warranted at all); HIGH needs at least 3 of each. In both tiers, only include a decision/risk/diagram that's genuinely load-bearing — padding to hit a number defeats the point of the minimum, which is auditability, not volume.

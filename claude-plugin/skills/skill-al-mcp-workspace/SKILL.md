...
- A build with analyzers on reports no analyzer warnings on a non-trivial project, or `al_getdiagnostics` returns another project's files (Gotcha 3)
...
| Diagnostics | Read the `al_compile` result — the authority. **al-mcp** `al_getdiagnostics` (by `filePath`/`folderPath`/`projectPath`) after `al_build` loses analyzer diagnostics — Gotcha 3 |
...
3. **`al_build` + `al_getdiagnostics` loses every analyzer diagnostic.** Observed on AL CLI 18.0.41.39415 / AL extension 18.0.2732683 (bc-dev 8.2, [issue #52](https://github.com/DSC-Group-Srl/dscgroup-bc-nav-agentic-dev/issues/52)): `al_build` with `enableCodeAnalysis=true` and the Full Analyzer Set, then `al_getdiagnostics`, returned one `AL0792` hint over 45 objects; `al_compile` on the same project with the same `codeAnalyzers` returned hundreds (`LC0091`×265, `AA0248`×101, `AC0014`/`AC0015`×30, `DC0007`×23, …). Only `source:"Compiler"` entries survive; analyzer warnings and hints vanish whatever the `severities` filter, explicit all-severities included. The two can also disagree the other way (`al_build` fails, `al_getdiagnostics` shows zero errors).
   - **Collateral — the store is not isolated per project.** `al_getdiagnostics` with `folderPath=app` also returned ~98 errors from sibling `app-test/`. Keep only diagnostics whose file lies under the requested project path before counting or reporting.
   - **Workaround:** counts and verdicts come from `al_compile` (or `Bash: al compile` with `/analyzer:`), never from build diagnostics — `compiler-authority-protocol.md` §0. `al_build` is for the `.app` only.
   - **Re-check before removing the workaround** (new AL CLI — `al --version`): on one non-trivial project with analyzers on, compare the analyzer-coded count from `al_compile` with that from `al_build` + `al_getdiagnostics` (no `severities`, filtered to the project). Equal → the defect is fixed upstream; drop this gotcha and the §0 bullet. Not scripted: both sides are MCP tools, not shell verbs.
